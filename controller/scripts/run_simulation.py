...
def run_simulation():
    ...
    for step in tqdm(range(duration)):
        ...
        if REJUVINATION_INTERVAL > 1 and step % REJUVINATION_INTERVAL != 0:
            ...
            ChartNonBlockedProxyRatio.objects.create(value=nonblocked_ratio, creation_time=step)
            nonblocked_count = all_active_proxies_count - blocked_proxies_count
            ChartNonBlockedProxyCount.objects.create(value=nonblocked_count, creation_time=step)
            flagged_count = Client.objects.filter(flagged=True, is_censor_agent=False).count()
            ...
            try:
                connected_user_ratio = (total_count - flagged_count - affected_clients_this_step) / total_count
            except:
            ...
            ChartNonBlockedProxyCount.objects.create(value=Proxy.objects.filter(is_active=True).count(), creation_time=step)
            flagged_count = Client.objects.filter(flagged=True, is_censor_agent=False).count()
            total_count = Client.objects.all().count()
            try:
