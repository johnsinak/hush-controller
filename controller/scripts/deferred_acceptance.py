    ...
    while waiting_clients:
        for student in waiting_clients.copy():
            ...
            if len(assignment_results[choice]) < capacities[choice]:
                assignment_results[choice] = get_sorted_results_with_student(student, choice)
                waiting_clients = get_waiting_list_without_student(student)
            else:
                if proxy_prefrences[choice].index(student) < proxy_prefrences[choice].index(assignment_results[choice][-1]):
                    assignment_results[choice] = get_sorted_results_with_student(student, choice)
                    waiting_clients = get_waiting_list_without_student(student)
